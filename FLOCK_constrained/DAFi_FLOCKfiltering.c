...
#define UPPERBOUND_NUM_CLUSTER 1000000  //upper bound of number of hyper regions
...
void get_spec_num(FILE *f_spec, int *num_rows)
{
	...
	printf("Number of Dimensions to be filtered is %d\n",number_rows);
	*num_rows=number_rows;
}

void get_spec_info(FILE *f_spec, int *filtered_d, int *filtered_low, int *filtered_upper,int num_rows)
{
	int temp=0;
	char line[LINE_LEN];
	...
	{
		sscanf(line,"%d\t%d\t%d\n",&filtered_d[temp],&filtered_low[temp],&filtered_upper[temp]);
		line[0]='\0';
		temp++;
	}

	if (temp==num_rows)
		printf("The last row of the spec file is %d\t%d\t%d\n", filtered_d[temp-1],filtered_low[temp-1],filtered_upper[temp-1]);
...
void tran(double **orig_data, int file_Len, int num_dm, int norm_used, double **matrix_to_cluster)
{
  ...
  if (norm_used==3) //0-1 min-max normalization
   {
      for (j=0;j<num_dm;j++)
        {
           for (i=0;i<file_Len;i++)
	...
	//max_num_clusters will be used to limit the number of dense hyper grids when selecting the density threshold
{
 ...
 // double avr_index=0;
  ...
  if (temp_num_dense_grids>UPPERBOUND_NUM_CLUSTER)
  {
	  //modified on July 23, 2010
...
///////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
double kmeans(double **Matrix, int k, double kmean_term, int file_Len, int num_dm, int *shortest_id, double **center, int random_init)
{
...
int main (int argc, char **argv)
{
  ...
  int num_rows=0;
  int Filtered=0; //Filtered == 0 means it is not a filtered mode; otherwise it is
  ...
  if ((argc!=2) && (argc!=4) && (argc!=5) && (argc!=7))
  {
	  ...
	  fprintf(stderr,"basic mode: FLOCK_Filtering data_file\n"); //modified on July 23, 2010
	  fprintf(stderr,"advanced mode 1: FLOCK_Filtering data_file num_bin density_index\n"); //modified on July 23, 2010
	  fprintf(stderr,"advanced mode 2: FLOCK_Filtering data_file num_bin density_index max_num_pop\n"); //added on Dec 16, 2010 for GenePattern
	  fprintf(stderr,"advanced mode 3: FLOCK_Filtering data_file num_bin density_index max_num_pop filter_spec_file filter_proportion\n");
	  printf("If you input less than 6 bins, K-means with your specified max_num_pop will be automatically executed.\n");
	  abort();
  }	
  ...
  if (argc==7)
  {
	  ...
	  filter_proportion=atof(argv[6]);

	  printf("filter_proportion=%f\n",filter_proportion);

	  get_spec_num(f_spec,&num_rows);

	  rewind(f_spec);

	  filtered_d=(int *)malloc(sizeof(int)*num_rows);
	  memset(filtered_d,0,sizeof(int)*num_rows);

	  filtered_low=(int *)malloc(sizeof(int)*num_rows);
	  memset(filtered_low,0,sizeof(int)*num_rows);

	  filtered_upper=(int *)malloc(sizeof(int)*num_rows);
	  memset(filtered_upper,0,sizeof(int)*num_rows);

	  get_spec_info(f_spec,filtered_d,filtered_low,filtered_upper,num_rows);
  ...
  kmeans(normalized_data, num_population, KMEANS_TERM, file_Len, num_dm, all_population_ID, population_center, kmeans_yes);
  }
  ...
  if (Filtered==1) //if it is the filtered mode, the population ID needs to be changed; only two clusters will be generated: events that are kept and events that are filtered out
  {
	  ...
	  filtered_out=(int *)malloc(sizeof(int)*file_Len);
	  memset(filtered_out,0,sizeof(int)*file_Len);
	  ...
	  filtered_p=(int *)malloc(sizeof(int)*num_population);
	  ...
	  for (i=0;i<file_Len;i++)
	  {
		  ...
		  {
			  if ((normalized_data[i][filtered_d[j]-1]<((double)filtered_low[j]/(double)DAG_BIN)) || (normalized_data[i][filtered_d[j]-1]>((double)filtered_upper[j]/(double)DAG_BIN)))
			  {
				  filtered_out[i]=1;
				  break;
			  }
	  ...
	  for (i=0;i<file_Len;i++)
		  if (filtered_out[i]==0)
			size_f[all_population_ID[i]]++;

	  for (i=0;i<num_population;i++)
		{
			if (size_c[i]!=0)
				percent_f[i]=(double)size_f[i]/(double)size_c[i]; //Note: this percent is actually a proportion value
			else
				percent_f[i]=0.0;
			
			if (percent_f[i]>=(double)(1.0000-filter_proportion)) //majority is inside the boundary, whole population needs to be kept
				filtered_p[i]=-1;

			if (percent_f[i]<=(double)filter_proportion) //majority is outside the boundary, whole population needs to be removed
				filtered_p[i]=1;
			
			if ((percent_f[i]>(double)filter_proportion) && (percent_f[i]<(double)(1.0000-filter_proportion))) //some events need to be filtered while others will be kept
				filtered_p[i]=0;
		}

	  f_majority=fopen("filter_proportion.txt","w");

	  fprintf(f_majority,"FLOCK_Population_ID\tProportion_in_Sample\tProportion_in_FilteringHyperRegion\tFiltered_P\n");

	  for (i=0;i<num_population;i++)
	  	  fprintf(f_majority,"%d\t%.4f\t%.4f\t%d\n",i+1,(double)size_c[i]/(double)file_Len,percent_f[i],filtered_p[i]);
	  
	  fclose(f_majority);

	  for (i=0;i<file_Len;i++)
	  {
		  pppp=all_population_ID[i];

		  if (filtered_p[pppp]>0) //this population needs to be discarded
			  all_population_ID[i]=1;
		  
		  if (filtered_p[pppp]<0) //this population needs to be kept
			  all_population_ID[i]=0;
		  
		  if (0==filtered_p[pppp]) //some will be filtered while others will be kept
			  all_population_ID[i]=filtered_out[i];
	  }
	  ...
	  for (i=0;i<file_Len;i++)
		  if (all_population_ID[i]==0) 
		  {
			  for (j=0;j<num_dm;j++)
				 if (j==num_dm-1)
					fprintf(f_filtered,"%d\n",(int)input_data[i][j]);
				 else
					fprintf(f_filtered,"%d\t",(int)input_data[i][j]);
		  }
