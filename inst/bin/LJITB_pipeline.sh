...
do
filename=$(basename "$f");
(ArrangeHeader.awk $preconfig/LJI_TB.header.lst $preconfig/header.txt $f > $filename)&
if [ $(($count % $numOfCores)) -eq 0 ]; then wait; fi
count=$((count+1));
done
wait
cd ..
cp $preconfig/LJI_TB.config pipeline.config
...
for file in $cwd/Preprocessed/*
	...
	if [ "$optimize" = true ] ; then
		dafi_intel $file $preconfig/LJI_TB.config $preconfig/Empty.config $numOfClusters $numOfReClusters $numOfCores $seed
	else
		dafi_gating $file $preconfig/LJI_TB.config $preconfig/Empty.config $numOfClusters $numOfReClusters $numOfCores $seed
	fi
	cd ..
done
