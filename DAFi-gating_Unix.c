...
//Please note that there can be only 1 row with flitered_output == 1
void get_spec_info(FILE *f_spec, int *filtered_ID, int *filtered_d_x, int *filtered_d_y, int *filtered_x_low, int *filtered_x_upper, int *filtered_y_low, int *filtered_y_upper, int *filtered_parent, int *filtered_type, int *filtered_output, int num_rows)
{
	int temp=0;
	char line[LINE_LEN];
	...
	{
		sscanf(line,"%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",&filtered_ID[temp],&filtered_d_x[temp],&filtered_d_y[temp],&filtered_x_low[temp],&filtered_x_upper[temp],&filtered_y_low[temp],
			&filtered_y_upper[temp],&filtered_parent[temp],&filtered_type[temp],&filtered_output[temp]);
		line[0]='\0';
		temp++;
	}

	if (temp==num_rows)
		printf("The last row of the spec file is %d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", filtered_ID[temp-1],filtered_d_x[temp-1],filtered_d_y[temp-1],filtered_x_low[temp-1],
		filtered_x_upper[temp-1],filtered_y_low[temp-1],filtered_y_upper[temp-1],filtered_parent[temp-1],filtered_type[temp-1],filtered_output[temp-1]);
...
///////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
double recurisve_optimize(double **Matrix, int k, double terms, int file_Len, int num_dm, int *shortest_id, double **center, int random_init)
{
...
int main (int argc, char **argv)
{
  ...
  if ((argc!=4) && (argc!=5))
  {
	  ...
	  fprintf(stderr,"DUnSup_Filtering data_file filter_spec_file1 filter_spec_file2\n"); //filter_spec_file 1 is those that need to be kept; filter_spec_file2 is those that need to be removed
	  fprintf(stderr,"DUnSup_Filtering data_file filter_spec_file1 filter_spec_file2 num_clusters\n");
	  abort();
  }	
  ...
  if (argc==4)
  {
	 max_num_pop=DEFAULT_NUM_POP; //default value, maximum 50 clusters
  }
  ...
  get_spec_info(f_spec,filtered_ID, filtered_d_x, filtered_d_y, filtered_x_low, filtered_x_upper, filtered_y_low, filtered_y_upper, filtered_parent, filtered_type, filtered_output,num_rows);
  get_spec_info(f_spec_2,filtered_ID_2,filtered_d_x_2, filtered_d_y_2, filtered_x_low_2, filtered_x_upper_2, filtered_y_low_2, filtered_y_upper_2, filtered_parent_2, filtered_type_2, filtered_output_2,num_rows_2);
  ...
  recurisve_optimize(normalized_data, num_population, TERMS, file_Len, num_dm, all_population_ID, population_center, 1);
  ...
  for (i=0;i<num_rows;i++) //for each predefined population i, generate population_ID, based on population_ID and filtered_parent, calculate number of events and percentage and write into file
  {
	  size_filtering[i]=0;
	  ...
	  if (filtered_type[i]==0) //clustering
	  {
		  ...
		  {
			  if ((population_center[j][filtered_d_x[i]-1]<((double)filtered_x_low[i]/(double)DAG_BIN)) || (population_center[j][filtered_d_x[i]-1]>((double)filtered_x_upper[i]/(double)DAG_BIN)) ||
				  (population_center[j][filtered_d_y[i]-1]<((double)filtered_y_low[i]/(double)DAG_BIN)) || (population_center[j][filtered_d_y[i]-1]>((double)filtered_y_upper[i]/(double)DAG_BIN)))
			  	filtered_p[j]=1;

			  if (i<num_rows_2) //there exists the reverse filtering configuration for this population
			  {
				  if ((population_center[j][filtered_d_x_2[i]-1]>=((double)filtered_x_low_2[i]/(double)DAG_BIN)) && (population_center[j][filtered_d_x_2[i]-1]<=((double)filtered_x_upper_2[i]/(double)DAG_BIN)) &&
				  (population_center[j][filtered_d_y_2[i]-1]>=((double)filtered_y_low_2[i]/(double)DAG_BIN)) && (population_center[j][filtered_d_y_2[i]-1]<=((double)filtered_y_upper_2[i]/(double)DAG_BIN)))
			  	filtered_p[j]=1;
			  }
		  }
		  ...
		  for (j=0;j<file_Len;j++)
		  {
			...
			if (filtered_p[pppp]>0) //this population needs to be discarded
				all_gate_ID[j][i]=1;
	  ...
	  if (filtered_type[i]==2) //ratio-based; FSC-A vs FSC-H or SSC-A vs SSC-H for singlets
	  {
		  for (j=0;j<file_Len;j++)
		  {
			  if (((normalized_data[j][filtered_d_y[i]-1]/normalized_data[j][filtered_d_x[i]-1])<((double)filtered_y_low[i]/(double)filtered_x_low[i])) || ((normalized_data[j][filtered_d_y[i]-1]/normalized_data[j][filtered_d_x[i]-1])>((double)filtered_y_upper[i]/(double)filtered_x_upper[i])))
				  all_gate_ID[j][i]=1;
			  else
			  {
				  if (i<num_rows_2)
				  {
					if ((normalized_data[j][filtered_d_x_2[i]-1]>=((double)filtered_x_low_2[i]/(double)DAG_BIN)) && (normalized_data[j][filtered_d_x_2[i]-1]<=((double)filtered_x_upper_2[i]/(double)DAG_BIN)) &&
					(normalized_data[j][filtered_d_y_2[i]-1]>=((double)filtered_y_low_2[i]/(double)DAG_BIN)) && (normalized_data[j][filtered_d_y_2[i]-1]<=((double)filtered_y_upper_2[i]/(double)DAG_BIN)))
						all_gate_ID[j][i]=1;
					else
						all_gate_ID[j][i]=0;				  
				  }
	  ...
	  if (filtered_type[i]==1) //bisecting
	  {
		  for (j=0;j<file_Len;j++)
		  {
			  if ((normalized_data[j][filtered_d_x[i]-1]<((double)filtered_x_low[i]/(double)DAG_BIN)) || (normalized_data[j][filtered_d_x[i]-1]>((double)filtered_x_upper[i]/(double)DAG_BIN)) ||
				  (normalized_data[j][filtered_d_y[i]-1]<((double)filtered_y_low[i]/(double)DAG_BIN)) || (normalized_data[j][filtered_d_y[i]-1]>((double)filtered_y_upper[i]/(double)DAG_BIN)))
				  all_gate_ID[j][i]=1;
			  else
			  {
				  if (i<num_rows_2)
	  ...
	  {
		  filtered_out=0;
		  final_gate_ID[j]=1; //1 means to remove

		  t=i;   //note that filtered_parent[0]=0;
		  while (t>=0) //the intersection of multiple gates needs to be calculated
		  {	 
		  ...
		  if (filtered_out==0) // && (all_gate_ID[j][i]==0) && (all_gate_ID[j][0]==0)) //neither removed by parent gates nor by the current gate
		  {
			  final_gate_ID[j]=0;	//0 means to keep
			  size_filtering[i]++;
		  }
	  } //end for j=0 to file_Len

	  ID2Center_all(input_data,file_Len,num_dm,2,final_gate_ID,gate_center);

	  //calculate percentage of a predefined cell population based on its parent
	  ...
	  if (filtered_output[i]==1) //generate flock_results.txt etc for visualization purpose. Note that the user should only specify to output only one predefined cell population; otherwise, the one defined later will overwrite the one defined previously
	  {
		  ...
		  for (j=0;j<file_Len;j++)
			...
			show(input_data, final_gate_ID, file_Len, 2, num_dm, para_name_string);

			//ID2Center_all(input_data,file_Len,num_dm,2,final_gate_ID,population_center);
