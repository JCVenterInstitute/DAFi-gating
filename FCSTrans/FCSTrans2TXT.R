...
anonymous <- function(name){
  if(is.null(nametable[[name]])){
    ...
    assign(name, paste("Sub",formatC(gbcount, width=3, flag="0"),sep = ""), envir = nametable)
  }
  nametable[[name]]
}
...
convertfcs <- function(fcs_raw, compen="internal") {
  ...
  if (keywords$FCSversion == "2" || keywords$FCSversion == "3" || keywords$FCSversion == "3.1") {
    ...
    if (datatype == 'F') {
      ...
      if(debug){print("Transformation via FCSTrans...")}
...
processfcsfile <- function(fcsfile, compen="internal", verbose=F, overwrite=F,
  ...
  print(paste("parameters:","verbose",verbose,"overwrite",overwrite,"renameSource",renameSource,"removeName",removeName,"copyRaw",copyRaw,"convert",convert,"savekeys",savekeys,sep = " "))
  ...
  if (isvalid) {
    ...
    tryCatch({
      ...
      if(grepl("_",tempFIL)){
        ...
        sampleNumber<-(unlist(strsplit(filpieces[length(filpieces)], ".")))[1]
      }else {
      ...
      if (removeName) {
        if (verbose) print(paste("    Removing Names", fcsfile))
        
        if(srcformat) {
          outputfilename<-paste(tubeName,paste(srcpieces[2:length(srcpieces)], collapse='_'),sampleNumber,sep="_")
      ...
      if (verbose) print (paste("    ! Error in read.FCS", ex))
      output=NULL
    })
  }else {
...
ipconvert <- function(entry, compen="internal", verbose=F, overwrite=F, renameSource=F, removeName=F, copyRaw=F, convert=T, savekeys=T, level=1) {
  ...
  if (is.vector(entry) && length(entry) > 1) {
    ...
    sapply(entry, ipconvert, verbose=verbose, overwrite=overwrite,
           renameSource=renameSource, removeName=removeName, copyRaw=copyRaw, convert=convert, savekeys=savekeys, level=level+1)
        ...
        if (length(files) > 0) {
          ...
          if (file.info(files[1])$isdir) {
            ...
            sapply(files, ipconvert, level=level+1, verbose=verbose,
                   overwrite=overwrite, renameSource=renameSource, removeName=removeName, copyRaw=copyRaw, convert=convert, savekeys=savekeys)
          } else {
